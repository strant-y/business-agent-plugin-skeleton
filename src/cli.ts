...
async function main(): Promise<void> {
  ...
  switch (command) {
    ...
    case 'impact':
      ...
      break;
    case 'capture': {
      // `--refresh-knowledge` is a value-less flag generated by the post-commit
      // hook; peel it off before option parsing so it is not misread as a value.
      const refreshKnowledge = args.includes('--refresh-knowledge');
      const optionArgs = args.filter((arg) => arg !== '--refresh-knowledge');
      const opts = parseCaptureOptions(optionArgs);
      const message = optionArgs
